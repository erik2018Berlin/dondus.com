...
const { name, notes, meetingIds, userId } = schema.tree
 ...
 * @apiParam meeting-slotIds Calendar's meeting-slotIds.
 * @apiParam userId Calendar's userId.
...
router.post('/',
  token({ required: true }),
  body({ name, notes, meetingIds, userId }),
  create)
 ...
 * @apiParam {String} access_token user access token.
 * @apiUse listParams
 * @apiSuccess {Object[]} calendars List of calendars.
 * @apiError {Object} 400 Some parameters may contain invalid values.
 * @apiError 401 user access only.
 */
router.get('/',
