...
export class CalendarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.calenderService.getAllCalendarsFromUser(this.currentUser)
      ...
      .subscribe(
        data => {
          if (data.length > 0){
            for (let i = 0; i < data[0].meetingIds.length; i++){
          this.calenderService.getMeeting_withId(data[0].meetingIds[i]).then(service => {
            this.calenderContent.push({title: service.title, id: service.id,
              image: 'http://dummyimage.com/169x185.png/5fa2dd/ffffff', description: service.description,
              date:  service.date.split('.')[0]});
  ...
  handleEventClick(arg): void {
    this.serviceService.getServiceWithId(arg.event.id)
      .subscribe(data => {
        this.util.toggleSidebar(data.title, data.id, 'http://dummyimage.com/169x185.png/5fa2dd/ffffff',
          data.description, data.price, data.category[0], data.postcodes);
      });
  }
